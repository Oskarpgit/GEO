...
function extractOfferId(url: string, pageText: string): string {
  ...
  try {
    const candidate = new URL(url).searchParams.get("offerId") ?? undefined;
    if (candidate && /^\d{8,}$/u.test(candidate)) fromQuery = candidate;
  } catch {
    // Keep parsing resilient for partial fixture URLs and use the page fallback below.
  }
  const fromText = pageText.match(/Numer oferty:\s*(\d{8,})/iu)?.[1];
  const offerId = fromUrl ?? fromQuery ?? fromText;
